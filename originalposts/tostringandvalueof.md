 ...
 * __Null__: The value `{@class=js}null`.
 * __Undefined__: The value `{@class=js}undefined`.
 * __Number__: All numbers, such as `{@class=js}0` and `{@class=js}3.14`.  Also `{@class=js}NaN`, and `{@class=js}Infinity`.
 * __Boolean__: The values `{@class=js}true` and `{@class=js}false`.
 * __String__: All strings, such as `{@class=js}"foo"` and `{@class=js}""`.

All other values are non-primitive, including arrays, functions, and plain old objects.  For completeness, here are the results of the `{@class=js}typeof` operator, applied to these values:
...
<span class="note">__Note__: `{@class=js}typeof null` should _not_ be `{@class=js}"object"`.  This is a mistake from the first versions of JavaScript, but it's really too late to fix.  A more sensible type would have been `{@class=js}"null"`, but this is what we're stuck with.</span>
...
Here we've defined the `valueOf` function of our `population` object to return the population, which should be a `{@class=js}Number`.
...
In this context, we're using the `+` operator to do string concatenation.  But, `foo` was _not_ converted to a string using `toString`, it was turned into a number using `valueOf`, then used for string concatenation.  This probably isn't what we want, but it is how it works.  It's a side-effect of the overloading of the `+` operator for arithmetic and string concatenation.  The `+`operator has a well-defined process:
 ...
 4. If either primitive value is a `{@class=js}String`, then skip to 7.
...
Since no _hint_ is passed to the `ToPrimitive` calls, the _hint_ will be defaulted to `{@class=js}Number` (unless it's a `{@class=js}Date`, which defaults to `{@class=js}String`).  This means that our `valueOf` function will be called, instead of `toString`.  It's not until _after_ the primitive values are retrieved that the interpreter decides whether it is going to do string concatenation or arithmetic.  That's why our example above returns `{@class=js}"5bar"` instead of `{@class=js}"foobar"`.
...
<span class="note">___Note 2___: I chose to use the `[].join` method because doing so was most likely to avoid any dead-code elimination optimizations in modern browsers.  I've had trouble with this before, in Firefox.  I _did_ try testing with the `{@class=js}String()` constructor, with similar results in most browsers.  Opera was an exception where using the explicit `toString` was close to __5x faster__.  In Firefox, the explicit cast was a bit faster, but both cases were about 100x faster than the `[].join` method (and other browsers), which means the code-path was probably being removed by the dead code eliminator.</span>
...
That the standard explicitly allows browsers to deviate here worried me.  Included in that test suite was a check that, in the absence of a _hint_, `{@class=js}Date` objects will default to `{@class=js}String` and `{@class=js}Boolean` objects will default to `{@class=js}Number`.  All browsers passed this check as well, which means that browser support for all of this functionality seems to be __consistent__ and __correct__.
