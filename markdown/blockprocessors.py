...
class BlockProcessor:
	...
	def getClassName(self, text):
		""" If the fist line is @@@something then make something the class attribute and strip the line """
		className = ""
		lines = text.split('\n')
...
class CodeBlockProcessor(BlockProcessor):
	...
	def run(self, parent, blocks):
		...
		if sibling and sibling.tag == "pre" and len(sibling) \
			...
			block, theRest = self.detab(block)
			block, className = self.getClassName(block)
			if className:
				code.set('class', className)
			code.text = markdown.AtomicString('%s\n' % block.rstrip())
		if theRest:
